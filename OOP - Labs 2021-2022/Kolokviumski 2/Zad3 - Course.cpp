...
private:
    int indx;
public:
    NoCourseException(int indx){
        this->indx=indx;
    }
    void message(){
        cout<<"Demonstratorot so indeks "<<indx<<" ne drzi laboratoriski vezbi"<<endl;
    }
};
...
public:
    ...
    int getBodovi(){
        ...
        return Student::getBodovi()+(float)(20*brojCasovi)/getBrojKursevi();
    }
    void pecati(){
        Student::pecati();
        cout<<": ";
...
void pecatiDemonstratoriKurs(char* kurs, Student** studenti, int n){
    Demonstrator *tmp = nullptr;
    for(int i=0;i<n;i++){
        tmp = dynamic_cast<Demonstrator *>(studenti[i]);
        if(tmp!=nullptr){
            for(int j=0;j<tmp->getBrojKursevi();j++){
                if(tmp->getKurs(i)==kurs){
                    tmp->pecati();
                    cout<<endl;
                    break;
                }
